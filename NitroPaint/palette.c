...
void doDiffuse(int i, int width, int height, unsigned int * pixels, int errorRed, int errorGreen, int errorBlue, int errorAlpha, float amt) {
	...
	unsigned int thisPixel = pixels[i];
	int l = LIGHTNESS(thisPixel);
	if (i % width < width - 1) {
		unsigned int right = pixels[i + 1];
		float mt = ((float) (255 - abs(LIGHTNESS(right) - l))) / 255.0f;
		mt *= mt;
		diffuse(right, errorRed * 7 * amt * mt / 16, errorGreen * 7 * amt * mt / 16, errorBlue * 7 * amt * mt / 16, errorAlpha * 7 * amt * mt / 16);
		pixels[i + 1] = right;
	}
	if (i / width < height - 1) {
		if (i % width > 0) {//downleft
			...
			float mt = ((float) (255 - abs(LIGHTNESS(right) - l))) / 255.0f;
			mt *= mt;
			diffuse(right, errorRed * 3 * amt * mt / 16, errorGreen * 3 * amt * mt / 16, errorBlue * 3 * amt * mt / 16, errorAlpha * 3 * amt * mt / 16);
		...
		if (1) {//down
			...
			float mt = ((float) (255 - abs(LIGHTNESS(right) - l))) / 255.0f;
			mt *= mt;
			diffuse(right, errorRed * 5 * amt * mt / 16, errorGreen * 5 * amt * mt / 16, errorBlue * 5 * amt * mt / 16, errorAlpha * 5 * amt * mt / 16);
		...
		if (i % width < width - 1) {
			...
			float mt = ((float) (255 - abs(LIGHTNESS(right) - l))) / 255.0f;
			mt *= mt;
			diffuse(right, errorRed * 1 * amt * mt / 16, errorGreen * 1 * amt * mt / 16, errorBlue * 1 * amt * mt / 16, errorAlpha * 1 * amt * mt / 16);
			pixels[i + width + 1] = right;
		}
	}
}
