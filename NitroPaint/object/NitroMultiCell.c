...
#include "NitroCell.h"
#include "NitroAnim.h"
#include "NitroMultiCell.h"

static int McbkIsValidNmcr(const unsigned char *buffer, unsigned int size);
static int McbkReadNmcr(NMCR *nmcr, const unsigned char *buffer, unsigned int size);

static const ObjIdEntry sFormats[] = {
	{
		FILE_TYPE_NMCR, NMCR_TYPE_NMCR, "NMCR",
		OBJ_ID_HEADER | OBJ_ID_SIGNATURE | OBJ_ID_CHUNKED | OBJ_ID_OFFSETS | OBJ_ID_VALIDATED,
		McbkIsValidNmcr,
		(ObjReader) McbkReadNmcr,
		NULL
	}
};

void McbkRegisterFormats(void) {
	ObjRegisterType(FILE_TYPE_NMCR, sizeof(NMCR), "Multi-Cell Animation", NULL, NULL);
