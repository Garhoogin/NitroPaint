		...
		case WM_COMMAND:
		{
			...
			if (notif == BN_CLICKED && (hWndControl == data->hWndOk || idc == IDOK)) {
				...
				if (writePalette) {
					...
					palette[0] = 0xFF00FF;
					RxGlbCreatePalette(px, width, height, palette + paletteOffset, paletteLength, &balance,
				...
				for (int i = 0; i < nObj; i++) {
					...
					RxGlbReduceImage(slice->px, indicesBuffer, width, height, palette + paletteOffset, paletteLength, 
						RX_FLAG_ALPHA_MODE_NONE | RX_FLAG_PRESERVE_ALPHA | RX_FLAG_NO_ALPHA_DITHER, diffuse, &balance);
